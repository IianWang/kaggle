...
-**我个人把本次筛选特征分为以下两点**
  ...
  - 特征共线性
### 4.1 特征性价比
**首先抛开其它的不想，主要考虑下一个特征在模型中拟合的过程中会出现哪些情况，我能想到第一个是特征不显著或者可以说显著性低，在模型中基本不起到什么作用；第二种可能性就是特征过于的杂乱，把该特征与标签关联并不能看出大致的走向和趋势或是其中夹杂着太多的噪声，对实际的拟合不起作用甚至造成干扰。**<br><br>
**刚刚是我所理解对特征性价比衡量的一个思想，也就是说加入这样一个特征令总体的综合水平不如之前，那么我就说这是一个性价比比较低的一个特征，至于怎样计算，我在这里引入一个较为好用的一个方法`正则化`**<br><br>
...
### 关于这个阈值怎么定，关于特征相关性强弱有这样的一个范围参考 `0.7 ~ 1`相关性强，`0.3 ~ 0.7`相关性中等，`0 ~ 0.3`相关性弱。不过比较麻烦的是，这个不是绝对的，当你去掉了一个你认为相关性为中等的特征，实际模型效果变差了或者是变好了，这如果不就实际情况而言实质上很难说，期待自己能够找到更好的筛选特征方法和技巧，就好比一门艺术。
## 5.交叉验证
